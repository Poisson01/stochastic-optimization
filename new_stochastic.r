...
for(aa in 1:times){
  ...
  # BIC 2nd in paper
  M_hat <- sort(M_hat); min_M <- M_hat; M <- c(); BIC_M <- 0; temp <- 20
  M_hat_beta <- solve(t(x[,M_hat]) %*% x[,M_hat]) %*% t(x[,M_hat]) %*% y
  M_hat_sigma <- (t(y - x[,M_hat] %*% M_hat_beta) %*% (y - x[,M_hat] %*% M_hat_beta)) / n
  L_0 <- min(eigen(t(x[,M_hat])%*%x[,M_hat])$values)
  beta_02 <- min(M_hat_beta^2)
  cut_off <- n * L_0 * beta_02 / 2 / M_hat_sigma
  M_hat_BIC_a <- -2 * sum(dnorm(y, x[,M_hat] %*% M_hat_beta, sqrt(M_hat_sigma), log = TRUE)) + (length(M_hat)) * log(n)
  #visited_s <- matrix(0,m,p)
  s <- c(1)
  s_beta_hat <- (solve(t(x[,s]) %*% x[,s])) %*% t(x[,s]) %*% y
  s_BIC <-  -2 * sum(dnorm(y, x[,s] %*% s_beta_hat, sqrt(M_hat_sigma), log = TRUE)) + (length(s)) * log(n)
  visited_BIC <- c()
  for(iter in 1:m){
    for(i in M_hat[-1]){
      plus_s <- rep(FALSE, p); minus_s <- rep(FALSE, p)
      ...
      if(plus_s[i] == TRUE){
        ...
        BIC_minus <-  -2 * sum(dnorm(y, x[,minus_s] %*% minus_beta_hat, sqrt(M_hat_sigma), log = TRUE)) + (sum(minus_s)) * log(n)
        visited_BIC <- append(visited_BIC,BIC_minus)
      }else {
        BIC_minus <- s_BIC
        plus_s[i] <- TRUE
        plus_beta_hat <- (solve(t(x[,plus_s]) %*% x[,plus_s])) %*% t(x[,plus_s]) %*% y
        BIC_plus <-  -2 * sum(dnorm(y, x[,plus_s] %*% plus_beta_hat, sqrt(M_hat_sigma), log = TRUE)) + (sum(plus_s)) * log(n)
        visited_BIC <- append(visited_BIC,BIC_plus)
      }
      if(sum(plus_s) <= n^(2/3)){
        if(abs(M_hat_BIC - BIC_plus) <= cut_off){
          M <- all_index[plus_s]
          BIC_M <- BIC_plus
      ...
      if(abs(M_hat_BIC - BIC_minus) <= cut_off){
        M <- all_index[minus_s]
        BIC_M <- BIC_minus
        if(length(M) < length(min_M)){
          min_M <- M
      ...
      prob <- 1/(1 + exp((-0.5 / temp * BIC_minus) + (0.5 / temp * BIC_plus)))
      z <- rbinom(1, 1, prob)
      if((sum(plus_s) > n ^ (2 / 3)) & (z == 1)){
